...
static int _yc_setschedparam(vthread_id_t id, struct vcategory *category)
{
    int ret = 0;
...
static int _yc_store_cgroup_cb(const char *sub_key, struct yconfig_json *sub_item, void *data)
{
    ...
    struct vcgroup *cgroups = loop_data->cgroups;
...
static int _yc_store_cgroups(vload_category_t category, struct vcategory *params)
{
    int ret = 0;
    struct yconfig_json *ctx = NULL;
    char key[VLOAD_KEY_SIZE] = {0};
    struct vcgroup *cgroups = params->cgroups;
...
vload_category_t vload_control_get_category_from_priority(int priority)
{
    ...
    for (index = 0; index < nb_categories; index++) {
        ...
        if (tmp_category == VLOAD_CATEGORY_UNDEFINED) {
            ...
            vapi_error("Invalid libvapi priority %d - load category undefined", priority);
            return tmp_category;
        }
    }

    vapi_error("Invalid libvapi priority %d", priority);
    return VLOAD_CATEGORY_INVALID;
}
...
int vload_control_add_to_category(vthread_id_t id, struct vcategory *category)
{
    ...
    if (vcgroups_add_to_cgroups(tid, category->cgroups)) {
        return -1;
    }
